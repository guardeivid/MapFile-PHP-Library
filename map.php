...
class Map {
  ...
  public function __construct($mapfile = NULL) {
    if (!is_null($mapfile) && file_exists($mapfile)) {
      $this->read($mapfile);
    }
  ...
  public function setFontSet($filename) {
    if (file_exists($filename)) {
      $this->fontsetfilename = $filename;
    } else {
      throw new Exception('FontSet file does not exists.');
    }
  }
  /**
  ...
  public function setSymbolSet($filename) {
    if (file_exists($filename)) {
      $this->symbolsetfilename = $filename;
    } else {
      throw new Exception('SymbolSet file does not exists.');
    }
  }
  ...
  private function read($mapfile) {
    ...
    while (($_sz = fgets($h, 1024)) !== false) {
      ...
      if (preg_match('/^MAP$/i', $sz)) {
        $map = TRUE;
      } else if ($map && is_null($reading) && preg_match('/^END( # MAP)?$/i', $sz)) {
        $map = FALSE;
      } else if ($map && is_null($reading) && preg_match('/^OUTPUTFORMAT$/i', $sz)) {
        $reading = 'OUTPUTFORMAT';
      } else if ($map && $reading == 'OUTPUTFORMAT' && preg_match('/^END( # OUTPUTFORMAT)?$/i', $sz)) {
        $reading = NULL;
      } else if ($map && $reading == 'OUTPUTFORMAT') {
        continue;
      } else if ($map && is_null($reading) && preg_match('/^QUERYMAP$/i', $sz)) {
        $reading = 'QUERYMAP';
      } else if ($map && $reading == 'QUERYMAP' && preg_match('/^END( # QUERYMAP)?$/i', $sz)) {
        $reading = NULL;
      } else if ($map && $reading == 'QUERYMAP') {
        continue;
      } else if ($map && is_null($reading) && preg_match('/^PROJECTION$/i', $sz)) {
        $reading = 'PROJECTION';
      } else if ($map && $reading == 'PROJECTION' && preg_match('/^END( # PROJECTION)?$/i', $sz)) {
        $reading = NULL;
        ...
        $this->symbolsetfilename = $matches[1];
